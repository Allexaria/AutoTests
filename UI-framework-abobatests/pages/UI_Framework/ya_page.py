...
class YaPage:
    ...
    def input_search(self, text):
        search_input = self.driver.find_element(
            By.XPATH, '//*[@class="search3__input mini-suggest__input"]'
        )
        search_input.clear()
    ...
    def find_button(self):
        find_button = WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable(
                (
                    By.XPATH,
                    '//*[@class="Button Button_view_action Button_width_auto Button_size_m CbirPanelMultimodalUrlForm-Button"]',
                )
            )
        )
        find_button.click()
    ...
    def search_button(self):
        search_button = WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable(
                (
                    By.XPATH,
                    '//*[@class="Button Button_view_action Button_width_auto Button_size_m CbirPanelMultimodalQuery-ButtonSearch"]',
                )
            )
        )
        search_button.click()
    ...
    def mts_music_open_page(self):
        ...
        mts_music_link = WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, '//span[contains(text(),"Слушать")]'))
        )
        ...
        try:
            accept_button = WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(
                    (
                        By.XPATH,
                        '//*[@class="button_wrapper__hyCP8 mtsds-c1-bold-upp-wide button_size__m__0hq4N button_alwaysWhite__HEPeo button_secondaryGray__IFHOC"]',
                    )
                )
            )
            accept_button.click()
    ...
    def sound_search_input(self, query):
        search_input = WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable(
                (By.XPATH, '//input[contains(@class, "input-search_input")]')
            )
        )
        search_input.click()
        search_input.clear()
        ...
        try:
            search_button = WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(
                    (By.XPATH, '//button[contains(@class, "input-search_icon__")]')
                )
            )
            search_button.click()
        except TimeoutException:
        ...
        WebDriverWait(self.driver, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, '//div[contains(@class,"track-row_track_cover_wrapper")]')
            )
        )
    ...
    def play_button(self):
        play_buttons = WebDriverWait(self.driver, 10).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, '(//div[contains(@class, "track-row_track_cover_wrapper")])[1]')
            )
        )
        if play_buttons:
        ...
        try:
            not_now_button = WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(
                    (
                        By.XPATH,
                        '//*[@class="Button Distribution-Button Distribution-ButtonClose Distribution-ButtonClose_view_button Button_view_default Button_size_m"]',
                    )
                )
            )
