...
# Group/API settings (token must be passed at build time)
...
RUN test -n "$GITLAB_API_TOKEN" || (echo "GITLAB_API_TOKEN build arg is required" && exit 1) && \
...
CMD ["pytest", "AutoTests-abobatests/tests", "--alluredir=allure-results"]
